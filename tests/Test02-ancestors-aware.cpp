...
class AncestorsAwareTest : public flw::AncestorsAware<Ts...> {
public:
  template <typename... Values> AncestorsAwareTest(const Values &...ancestors) {
...
TEST(AncestorsAware, quadruple_bind_check) {
  ...
  AncestorsAwareTest<int, float, int, std::string> sample(val0, val1, val2,
                                                          val3);
