...
template <typename T> class SourceTest : public flw::Source<T> {
public:
  SourceTest() : flw::Source<T>(""){};
};

template <typename T, typename... Ts>
class NodeTest : public flw::Node<T, Ts...> {
public:
  template <typename... Values>
  NodeTest(const std::function<T(const Ts &...)> &evaluation,
...
TEST(SourcesNodes, twoSources_oneNode) {
  ...
  NodeTest<int, int, float> node(
      [](const int &i1, const float &i2) { return 0; }, source0, source1);
...
TEST(SourcesNodes, twoSources_twoNode) {
  ...
  NodeTest<int, int, float> node0(
      [](const int &i1, const float &i2) { return 0; }, source0, source1);
  NodeTest<int, int, float> node1(
      [](const int &i1, const float &i2) { return 0; }, source0, source1);
